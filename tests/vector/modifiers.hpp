...
/*   Updated: 2021/10/07 16:36:30 by badam            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef TESTS_VECTOR_MODIFIERS_HPP
# define TESTS_VECTOR_MODIFIERS_HPP
...
int	vector_erase(container &vector)
{
	...
	if (ret != vector.begin() || vector.size() != 1 || vector.empty())
		return (3);
	ret = vector.erase(vector.begin());
	if (ret != vector.begin() || vector.size() != 0 || !vector.empty())
		return (4);
	...
	if (ret != vector.end() || vector.size() != 1 || vector.empty())
		return (5);
	ret = vector.erase(vector.begin());
	...
	ret = vector.erase(pos);
	if (ret != vector.end() || vector.size() != 3 || vector.empty())
		return (7);
	ret = vector.erase(vector.begin());
	if (ret != vector.begin() || vector.size() != 2 || vector.empty())
		return (8);
...
int	vector_insert_1(container &vector)
{
	...
	if (ret != pos || vector.size() != 2 || vector.empty())
		return (2);
		
	ret = vector.insert(pos, 3);
	std::advance(pos = vector.begin(), 1);
	if (ret != pos || vector.size() != 3 || vector.empty())
		return (3);


	ret = vector.insert(vector.end(), 7);
	std::advance(pos = vector.end(), -1);
	if (ret != pos || vector.size() != 4 || vector.empty())
		return (4);

	ret = vector.insert(pos, 6);
	std::advance(pos = vector.end(), -2);
	if (ret != pos || vector.size() != 5 || vector.empty())
		return (5);

	ret = vector.insert(pos, 5);
	std::advance(pos = vector.end(), -3);
	if (ret != pos || vector.size() != 6 || vector.empty())
		return (6);


	ret = vector.insert(vector.begin(), 0);
	if (ret != vector.begin() || vector.size() != 7 || vector.empty())
		return (7);

	std::advance(pos = vector.begin(), 1);
	ret = vector.insert(pos, 1);
	std::advance(pos = vector.begin(), 1);
	if (ret != pos || vector.size() != 8 || vector.empty())
		return (8);
		
	ret = vector.insert(vector.end(), 8);
	std::advance(pos = vector.end(), -1);
	if (ret != pos || vector.size() != 9 || vector.empty())
		return (9);
