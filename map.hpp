...
{
	protected:
		typedef		pair<const Key, T>	_item;
	...
	public:
		...
		map_node(const _item &val, Alloc *alloc, const void *tip = NULL): data(NULL), red(true), parent(NULL), left_child(NULL), right_child(NULL)
		{
...
{
	typedef map																_self;
	typedef	ft::core< pair<const Key, T>, Alloc, map_node<Key, T, Alloc>, map_iterator< pair<const Key, T>, map_node<Key, T, Alloc> >, map_iterator< const pair<const Key, T>, map_node<Key, T, Alloc> > >	_parent;
	...
	protected:
		typedef map_node<Key, T, Alloc>		node;
		...
		bool	_dump(node *n, size_t indent = 0)
		{
			...
			if (n)
			{
				std::cout << "- " << n->data->first << " (=" << n->data->second << ") - " << (_is_red(n) ? "Red" : "Black");
				if (n->parent)
					std::cout << " - parent is [" << n->parent->data->first << "]";
				std::cout << std::endl;
	...
	public:
		...
		pair<iterator, bool>	insert(const value_type &val)
		{
			...
			else
			{
				...
				{
					parent = *insert_point;
					if (key_comp()(val.first, (*insert_point)->data->first))
